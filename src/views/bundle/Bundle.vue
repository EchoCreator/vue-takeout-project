...
const rules = {
  name: [
    { required: true, message: "套餐名不能为空", trigger: ["blur", "change"] },
  ],
  category: [
    {
      required: true,
...
const setDishes = (row) => {
  ...
  dishesData.value.bundleDishList = JSON.parse(
    JSON.stringify(row.bundleDishList)
  );
  bundelDishes.value = JSON.parse(JSON.stringify(row.bundleDishList));
  inputDishValue.value = "";
  for (let index in bundelDishes.value) {
    inputDishVisible.value.push(false);
  }
};
...
const editDishes = async () => {
  for (let index in bundelDishes.value) {
    if (
      bundelDishes.value[index].name === "" ||
      bundelDishes.value[index].value === ""
    ) {
      ElMessage.closeAll();
      ElMessage({
        message: "请补充完已有的套餐菜品或标签",
  ...
  if (bundelDishes.value.length !== 0) {
    dishesData.value.bundleDishList = bundelDishes.value;
  }
...
// 修改套餐菜品
const inputDishRef = ref();
const inputDishValue = ref("");
const inputDishVisible = ref([]);

const bundelDishes = ref([]);
const handleDishesTagClose = (tag, index) => {
  const list = bundelDishes.value[index].value
    .split(",")
    .filter((item) => item.trim() != "");
  list.splice(list.indexOf(tag), 1);
  bundelDishes.value[index].value = list.join(",");
};

const showDishesInput = (index) => {
  if (bundelDishes.value[index].name === "") {
    ElMessage({
      message: "菜品名称不能为空！",
      customClass: "custom-message",
      type: "error",
  ...
  inputDishVisible.value[index] = true;
  nextTick(() => {
    if (inputDishRef.value && inputDishRef.value.input) {
      inputDishRef.value.input.focus();
    }
  });
};

const handleDishesInputConfirm = (index) => {
  if (inputDishValue.value) {
    bundelDishes.value[index].value += "," + inputDishValue.value;
  }
  inputDishVisible.value[index] = false;
  inputDishValue.value = "";
};

const addBundleDishes = () => {
  const length = bundelDishes.value.length;
  if (length === 5) {
    ElMessage({
      message: "每份套餐最多只能有五种菜品！",
      customClass: "custom-message",
      type: "error",
    });
    return;
...
const deleteBundleDishes = (index) => {
  bundelDishes.value.splice(index, 1);
};
...
const reset = () => {
  ...
  dishesData.value.bundleDishList = [];
  bundelDishes.value = [];
  inputDishValue.value = "";
  inputDishVisible.value = [];
};
      ...
      <el-tabs v-model="activeName" @tab-change="handleChange">
        ...
        >
          ...
          >
            ...
            <el-table-column prop="name" label="套餐名" width="260" />
    ...
    >
      ...
      <el-form-item label="套餐菜品">
        <el-table
          ...
          empty-text="该套餐还没有菜品"
        >
          <el-table-column label="菜品类别（如酒水饮料等）" width="250">
            <template #default="{ row }">
              <el-input
                v-model="row.name"
                style="width: 100px"
          ...
          <el-table-column label="菜品名（如橙汁、可乐、啤酒等）">
            <template #default="scope">
              ...
              <el-input
                ref="inputDishRef"
                v-if="inputDishVisible[scope.$index]"
                v-model="inputDishValue"
                size="small"
                style="width: 100px"
                maxlength="7"
                @keyup.enter="handleDishesInputConfirm(scope.$index)"
                @blur="handleDishesInputConfirm(scope.$index)"
              />
              <el-button
                v-else
                class="button-new-tag"
                size="small"
                @click="showDishesInput(scope.$index)"
              >
                + 添加标签
              </el-button>
            </template>
          ...
          <el-table-column label="操作" width="100">
            <template #default="scope">
              <el-button type="danger" @click="deleteBundleDishes(scope.$index)"
                >删除</el-button
              >
            </template>
          </el-table-column>
        </el-table>
        <el-button type="success" class="add-flavor" @click="addBundleDishes"
          >添加套餐菜品</el-button
        >
      </el-form-item>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button
          color="#626aef"
          type="primary"
          @click="dialogTitle === '添加套餐' ? addDishes() : editDishes()"
        >
          确定
        </el-button>
      </div>
